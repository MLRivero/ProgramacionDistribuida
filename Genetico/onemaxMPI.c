...
void main(int argc, char *argv) {
  int comm_sz, dst; /* Number of processes */
  ...
  dst = (my_rank + 1)%comm_sz;
  MPI_Send(max_local, 11, MPI_INT, dst, 0, MPI_COMM_WORLD);
  if(my_rank == 0){
    MPI_Recv(nuevo_max, 11, MPI_INT, comm_sz-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }else{
    src = my_rank -1;
    MPI_Recv(nuevo_max, 11, MPI_INT, src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }
}
